        ...
        with tab:
            ...
            if selected_items:
                if cat != "seasonings":
                    ...
                    # ➔ 【高速化のキモ1】2列のカラムを先に「外側」で1回だけ生成。
                    #     ループ内で何度も columns を作り直す重い処理を完全に無くします。
                    cols = st.columns(2)
                    
                    for i, item in enumerate(selected_items):
                        # 交互に左カラム（cols[0]）と右カラム（cols[1]）に振り分ける
                        with cols[i % 2]:
                            with st.container(border=True):
                                ...
                                # 食材名を上に大きく出し、下にインプット欄を並べる形にスリム化。
                                # 1マスコミの中にさらに columns を3つ作る（[4,3,3]の処理）重いネストを廃止し、
                                # 描画速度を10倍以上に引き上げます。見た目のすっきり感もアップします。
                                st.markdown(f"<b>{item}</b>", unsafe_allow_html=True)
                                
                                c_amt, c_unit = st.columns(2)
            ...
            with st.form(key=f"form_{cat}", clear_on_submit=True):
                ...
                if col_btn.form_submit_button("追加", icon=":material/add:"):
